...
100.0
```
A file of this type is generated by the findOptimalTemps.py script. The most principled way to run AdmixtureBayes is to use this option and ignore the *MCMC_chains*  and *maxtemp* options. These other two options are provided mostly to provide compatability with earlier versions of AdmixtureBayes and reproducibility of analyses using these older versions. See the section below on MC<sup>3</sup> Mixing for more details. For those curious about the exact MC<sup>3</sup> details, these temperatures are the temperatures with which we heat the likelihood function. The prior function is heated with a separate temperature scheme, where the temperatures are heated on a linear scale from 1.0 to 1.3 inclusive.
...
2) Improve the swap rates between the heated chains of the MC<sup>3</sup> algorithm. The particular benefit of  MC<sup>3</sup> is that it helps the sampler traverse regions of the state space with low probability in order to help sample different local maxima. However, this only works properly if chains of adjacent temperatures swap at an appreciable rate. This requires the exact temperatures of the heated chains to be set precisely, and the temperatures which give optimal mixing will depend on the specific dataset being analyzed. Therefore, the most principled way to set these temperatures is to dynamically set them in a dataset-specific manner by measuring the swap rates between chains. We provide a script to accomplish this, findOptimalTemps.py, which can be run with the following command:
