...
f.close
thinnedindex = 0
for treerow in (range(round(0.35*len(df.index)),len(df.index), round( 0.65*len(df.index)/ 120)  ) ):
    chosen_topology = ((df[["tree"]]).iloc[treerow])[0]
    fff = open("TemporaryFiles/add" + str(thinnedindex) + ".txt", "w")
    fff.write(str(((df[["add"]]).iloc[treerow])[0] ) +  "\n")
    fff.close
    ...
    f = open("TemporaryFiles/Tree" + str(thinnedindex) +  ".txt", "w")
    thinnedindex = thinnedindex + 1
    for node in tree:
        if (tree[node][2] is not None):
            for adds in admixture_proportion_intervals:
                for branchh in branches_intervals:
