...
LCM = 60
treemixindices = c(4,5 , 16)
LL = rep(treemixindices, each = LCM/length(treemixindices))
for (jj in 1:LCM) {
convertoutput(
 paste0("TemporaryFiles/treeoutput",toString(LL[jj]) , ".edges" ),
 paste0("TemporaryFiles/treeoutput",toString(LL[jj]) , ".vertices" ),
 paste0("TemporaryFiles/TREEMIXoutput",toString(jj) , ".txt" ),
 paste0("TemporaryFiles/TREEMIXoutgroup",toString(jj) , ".txt" ))
}

orienttindices =  1:20
LL = rep(orienttindices, each = LCM/length(orienttindices))

for (jj in 1:LCM) {
convertoutput(
 paste0("TemporaryFiles/orientoutput",toString(LL[jj]) , ".edges" ),
 paste0("TemporaryFiles/orientoutput",toString(LL[jj]) , ".vertices" ),
