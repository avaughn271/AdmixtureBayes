...
EDGES1 = cbind(read.table("TemporaryFiles/TREEMIXoutput.txt")[,1],read.table("TemporaryFiles/TREEMIXoutput.txt")[,2])   
...
TreeMIXResult = checkequality(EDGES1 , EDGES2, EDGES2)

EDGES1 = cbind(read.table("TemporaryFiles/ORIENToutput.txt")[,1],read.table("TemporaryFiles/ORIENToutput.txt")[,2])   
EDGES2 = cbind(read.table("TemporaryFiles/TrueTree.txt")[,1], read.table("TemporaryFiles/TrueTree.txt")[,2])   
for (i in 1:length(EDGES2)) {
  if (EDGES2[i] %in% EDGES2[,2]) EDGES2[i] = paste0(EDGES2[i], "_xx")
}
ORIENTResult = checkequality(EDGES1 , EDGES2, EDGES2)

write.table(data.frame(ORIENTResult, TreeMIXResult ), file = "TemporaryFiles/TopEq.txt",
            row.names = F, quote = F)