...
for (jjj in 1:100) {
    ...
    EDGES1 = cbind(read.table(paste0("TemporaryFiles/Tree", toString(jjj), ".txt" ))[,1],read.table(paste0("TemporaryFiles/Tree", toString(jjj), ".txt" ))[,2])   
    EDGES2 = cbind(read.table("TemporaryFiles/TrueTree.txt")[,1], read.table("TemporaryFiles/TrueTree.txt")[,2])   
    for (i in 1:length(EDGES2)) {
      if (EDGES2[i] %in% EDGES2[,2]) EDGES2[i] = paste0(EDGES2[i], "_xx")
    }
    results = c(results,  checkequality(EDGES1 , EDGES2, EDGES2))
}
MEAN = mean(results[1:100])
write.table(data.frame( MAP, MEAN ), file = "TemporaryFiles/TopEq.txt",
            row.names = F, quote = F)